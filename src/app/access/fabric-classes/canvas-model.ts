...
export class CanvasModel {
	...
	setCanvasMouseDown(func) {
		this.canvas.on('mouse:down', function (options) {
			func(options);
			//////window.setTimeout(function() {window.scope.$apply();}, 0);
		});
	}
	setCanvasMouseUp(func) {
		this.canvas.on('mouse:up', function (options) {
			func(options);
			//window.setTimeout(function() {window.scope.$apply();}, 0);
		});
	}
	setCanvasMouseMove(func) {
		this.canvas.on('mouse:move', function (options) {
			func(options);
			//window.setTimeout(function() {window.scope.$apply();}, 0);
		});
	}
	setCanvasGroupSelection(func) {
	...
	setCanvasParam(params) {
		this.canvas.set(params);
	}

	// MUST be overridden
	getMouseTopLeftPos(options) {
		if (this.getPosMouse) {
			return this.getPosMouse(options);
		}
		return {
